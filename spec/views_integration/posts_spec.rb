...
RSpec.describe Post, type: :system do
    ...
    let!(:post) {Post.create({ author: user, title: 'First Post', text: 'My first post', comments_counter: 0, likes_counter: 1}) }

    let!(:comment1){Comment.create({user: user, post: post, text: 'First comment'})}
    let!(:comment2){Comment.create({user: user, post: post, text: 'Second comment'})}
    let!(:comment3){Comment.create({user: user, post: post, text: 'Third comment'})}
    let!(:comment4){Comment.create({user: user, post: post, text: 'Fourth comment'})}
    let!(:comment5){Comment.create({user: user, post: post, text: 'Fifth comment'})}
    let!(:comment6){Comment.create({user: user, post: post, text: 'Sixth comment'})}
